...
class Win720:
    ...
    def buy_Win720(self, auth_ctrl: auth.AuthController,) -> dict:
        ...
        orderNo, orderDate = self._doOrderRequest(
            auth_ctrl, win720_round, extracted_num
        )

        body = json.loads(
            self._doConnPro(auth_ctrl, win720_round, extracted_num, orderNo, orderDate)
        )
    ...
    def _makeAutoNumbers(
        self, auth_ctrl: auth.AuthController, win720_round: str
    ) -> str:
        payload = "ROUND={}&SEL_NO=&BUY_CNT=&AUTO_SEL_SET=SA&SEL_CLASS=&BUY_TYPE=A&ACCS_TYPE=01".format(
            win720_round
        )
        headers = self._generate_req_headers(auth_ctrl)
    ...
    def _doOrderRequest(
        self, auth_ctrl: auth.AuthController, win720_round: str, extracted_num: str
    ) -> str:
        payload = "ROUND={}&AUTO_SEL_SET=SA&SEL_CLASS=&SEL_NO={}&BUY_TYPE=M&BUY_CNT=5".format(
            win720_round, extracted_num
        )
        headers = self._generate_req_headers(auth_ctrl)
    ...
    def check_winning(self, auth_ctrl: auth.AuthController) -> dict:
        ...
        result_data = {
            ...
            "money": ",".join(
                [
                    winnings[6 + (i * 8)].text.strip()
                    for i in range(0, int(len(winnings) / 7))
                ]
            ),
